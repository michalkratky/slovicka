...
// Get database instance for selected database
...
async function initializeDatabase() {
  try {
    ...
    for (const [key, config] of Object.entries(DATABASE_CONFIG.databases)) {
      ...
      await db.connect();
      console.log(
        `✅ Database ${config.name} (${config.file}) connected successfully`,
      );
    }
    return true;
  } catch (error) {
...
app.get("/api/health", (req, res) => {
  ...
  const db = getDatabase(selectedDb);
  res.json({
    status: "ok",
  ...
  try {
    ...
    if (!wordId || !userAnswer || !targetLanguage) {
      return res.status(400).json({
        error: "Missing required fields: wordId, userAnswer, targetLanguage",
      });
    }
...
app.post("/api/next-word", async (req, res) => {
  try {
    ...
    if (!enabledGroups || !translationDirections) {
      return res.status(400).json({
        error: "Missing required fields: enabledGroups, translationDirections",
      });
    }

    // Get enabled group keys (those with value = true)
    const enabledGroupKeys = Object.keys(enabledGroups).filter(
      (key) => enabledGroups[key],
    );
    ...
    // Get next word
    const nextWord = await db.getNextWordByDifficulty(
      enabledGroupKeys,
      directions,
    );
...
// Rate limit for OpenAI validation to prevent cost escalation
...
app.post("/api/validate-translation", validationLimiter, async (req, res) => {
  try {
    const { wordId, userAnswer, targetLanguage } = req.body;
    ...
    const correctTranslation =
      targetLanguage === "slovak" ? word.slovak : word.english;
    ...
    if (validation.isValid && validation.confidence >= 0.7) {
      ...
      const updatedAnswers = await db.getCorrectAnswers(
        parseInt(wordId),
        targetLanguage,
      );
...
app.post("/api/words", async (req, res) => {
  try {
    ...
    if (!slovak || !english || !category) {
      return res
        .status(400)
        .json({ error: "Missing required fields: slovak, english, category" });
    }
    ...
    if (synonyms) {
      if (synonyms.slovak && !Array.isArray(synonyms.slovak)) {
        return res
          .status(400)
          .json({ error: "Slovak synonyms must be an array" });
      }
      if (synonyms.english && !Array.isArray(synonyms.english)) {
        return res
          .status(400)
          .json({ error: "English synonyms must be an array" });
      }
    }
...
app.put("/api/words/:id", async (req, res) => {
  try {
    ...
    if (updates.synonyms) {
      ...
      if (
        updates.synonyms.english &&
        !Array.isArray(updates.synonyms.english)
      ) {
        return res
          .status(400)
          .json({ error: "English synonyms must be an array" });
    ...
    res.json({
      ...
      summary: {
        ...
        averageCorrect:
          history.length > 0
            ? Math.round(
                history.reduce((sum, day) => sum + day.correct_answers, 0) /
                  history.length,
              )
            : 0,
        averageIncorrect:
          history.length > 0
            ? Math.round(
                history.reduce((sum, day) => sum + day.incorrect_answers, 0) /
                  history.length,
              )
            : 0,
        totalTimeMinutes: history.reduce(
          (sum, day) => sum + day.total_time_minutes,
          0,
        ),
      },
    });
...
app.post("/api/record-answer", async (req, res) => {
  try {
    ...
    if (!wordId || !direction || typeof isCorrect !== "boolean") {
      return res.status(400).json({
        error: "Missing required fields: wordId, direction, isCorrect",
      });
    }
    ...
    if (!["sk-en", "en-sk"].includes(direction)) {
      return res
        .status(400)
        .json({ error: "Invalid direction. Must be sk-en or en-sk" });
    }
    ...
    console.log(
      `Recording answer: wordId=${wordId}, direction=${direction}, correct=${isCorrect}`,
    );
  ...
  try {
    ...
    for (const duplicate of duplicates) {
      ...
      await new Promise((resolve, reject) => {
        db.db.run(
          "DELETE FROM session_statistics WHERE session_date = ?",
      ...
      console.log(
        `Consolidated ${duplicate.duplicate_count} entries for ${duplicate.session_date}`,
      );
    }
...
app.post("/api/import-words", async (req, res) => {
  try {
    ...
    if (!Array.isArray(words) || !category) {
      return res.status(400).json({
        error: "Invalid request. Expected array of words and category",
      });
    }
    ...
    for (const word of words) {
      try {
        ...
        errors++;
        errorDetails.push(
          `Failed to import ${word.slovak}/${word.english}: ${error.message}`,
        );
      }
    }
...
module.exports = { app, initializeDatabase, getDatabase };
