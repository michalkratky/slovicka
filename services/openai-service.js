...
class OpenAIService {
  constructor() {
    ...
    if (!apiKey) {
      console.warn(
        "OpenAI API key not found. Translation validation will not work.",
      );
   ...
   * @param {string} languagePair - The database key (e.g., 'sk-en', 'de-en')
  ...
  async validateTranslation(
    ...
    if (!this.isEnabled) {
      return {
        isValid: false,
        explanation:
          "OpenAI service is not enabled. Please set OPENAI_API_KEY in your .env file.",
      };
    }
    ...
    try {
      ...
      const messages = [
        {
          role: "system",
          content: `You are a bilingual ${sourceLang}-${targetLang} language expert. Your task is to determine if a translation is valid.`,
        },
        {
          role: "user",
          content: `
...
`,
      ...
      console.log(
        `Translation validation for "${userTranslation}": ${result.isValid ? "Valid" : "Invalid"}`,
      );
